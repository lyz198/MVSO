...
class MVSOPPIS(nn.Module):
    def __init__(self, nlayers, nfeat, nhidden, nclass, dropout, lamda, alpha):
        super(MVSOPPIS, self).__init__()

        self.deep_agat = deepAGAT(nlayers=nlayers, nfeat=nfeat, nhidden=nhidden, nclass=nclass,
                                  dropout=dropout, lamda=lamda, alpha=alpha)
        self.graphppis = GraphPPIS(nlayers=nlayers, nfeat=nfeat, nhidden=nhidden, nclass=nclass,
        ...
        self.fcs.append(nn.Linear(1536, 256))
        # self.fcs.append(nn.Linear(256, 256))
        self.fcs.append(nn.Linear(256, 128))
        self.fcs.append(nn.Linear(128, nclass))
        self.mha = MultiHeadAttention(in_features=nhidden, head_num=1)
    ...
    def forward(self, x, graph, adj_matrix, seq_name):
        x = x.float()
        x = x.view([x.shape[0]*x.shape[1], x.shape[2]])

        # output1 = self.graphppis(x, adj_matrix, seq_name)
    
        output1 = self.subgraph(x, adj_matrix, 0)
        # output = output1
        output2 = self.deep_agat(x=x, graph=graph, efeats=graph.edata['ex'])
        # output = output2

        output1 = self.mha(output1.unsqueeze(0), output1.unsqueeze(0), output2.unsqueeze(0))############
        output1 = output1.squeeze(0)



         ### add big model
        chain_embedding = get_ProtT5_features(seq_name[0])
        chain_embedding = torch.tensor(chain_embedding)
        chain_embedding = Variable(chain_embedding.cuda(device))


        output = torch.cat((output1, output2, chain_embedding), dim=1)

        
        
        output = self.act_fn(self.fcs[0](output))
        output = F.dropout(output, DROPOUT, training=self.training)
